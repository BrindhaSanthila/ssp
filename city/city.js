/****************************** INSERT & UPDATE ***************************************/
function citycreation_cu(city_id,action)
{
/*	var img_name=document.getElementById("img_name").value;
	var img_name1=document.getElementById("img_name1").value;*/
	
	var action_type = "SUBMIT";
	
	if(action=="Add" || action=="Update" )
	{
			
          //  alert(file_data);	
			var form_data = new FormData();
			
			form_data.append("area_id", $("#area_id").val());
			form_data.append("city_name", $("#city_name").val());
			form_data.append("apprx_km", $("#apprx_km").val());			
			form_data.append("active_status", $("#active_status").val());
			form_data.append("city_id", city_id);
			
		}

		if(action=="Update"){
			action_type = "UPDATE";
		}
	
	var area_id = $("#area_id").val();
	var city_name = $("#city_name").val();
	var apprx_km = $("#apprx_km").val();

	if(area_id=='')
	{
		alert('Please Select Area');
		$("#area_id").focus();
	
	}else if(city_name=='')
	{
		alert('Please enter City Name');
		$("#city_name").focus();
	
	} else if(apprx_km=='')
	{
		alert('Please enter approximate km');
		$("#apprx_km").focus();
	
	}
else {
      $("#add").attr("disabled", "disabled");
		jQuery.ajax({
			type: "POST",
			url: "city/curd.php?action="+action_type,
			cache: false,
			contentType: false,
			processData: false,
			data: form_data,
			success: function(msg)
			{
				
				alert(msg);
				window.location="index.php?file=city/list";
			}
		});
	}
}


function district_list(state_id){
	jQuery.ajax({
			type: "POST",
			url: "customer/curd.php",
			data: "state_id="+state_id+"&action="+"district_list",
			success: function(msg){ 
				$("#district_id").html(msg);
			}
	});
}

function city_list(){
	
	var st_id = document.getElementById("state_id").value;
	var dis_id = document.getElementById("district_id").value;
	
	jQuery.ajax({
			type: "POST",
			url: "customer/curd.php",
			data: "stat_id="+st_id+"&dis_id="+dis_id+"&action="+"city_list",
			success: function(msg){ 
				$("#city_id").html(msg);
			}
	});
}






