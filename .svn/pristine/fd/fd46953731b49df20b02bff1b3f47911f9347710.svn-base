...
if($action == 'VIEW')
{
	...
	$pay_list = $pdo_conn->prepare("SELECT * FROM paymentfollowups WHERE executive ='".$_POST['staffcreation_id']."' AND  delete_status != '1' $query ");
	$pay_list->execute();
...
<table width="100%" cellspacing="0" cellpadding="0" >
	...
	<table width="100%" cellspacing="0" cellpadding="0" border="1" style="float:right;">
	 ...
	 { 
		...
		$pay_sum = $pdo_conn->prepare("SELECT SUM(today_collection),bal_amt FROM paymentfollowups WHERE executive ='".$_POST['staffcreation_id']."' AND  delete_status != '1' and date='".$value['date']."' and customer_name='".$value['customer_name']."' and company_name='".$value['company_name']."'");
		$pay_sum->execute();
		$paymentsum = $pay_sum->fetchAll();	
		$pending_amount = $paymentsum[0]['bal_amt'] -$paymentsum[0]['SUM(today_collection)'];?>
		<tr>
		<td align="left" height="25" class="style2">&nbsp;<?php echo $i = $i+1; ?>.</td>
		...
		<?php  $cncn_nam = $pdo_conn->prepare("SELECT * FROM ourconcerns WHERE concern_id='".$value['company_name']."' and delete_status!=1");
		$cncn_nam->execute();
		...
		<td align="right" class="style2">&nbsp;<?php echo number_format($paymentsum[0]['SUM(today_collection)'],2);  ?></td>
...
{	
	...
	$main_list = $pdo_conn->prepare("SELECT * FROM staffcreation WHERE staffcreation_id ='".$_POST['staffcreation_id']."' AND  delete_status != '1'");
	$main_list->execute();
	$mainlists = $main_list->fetch();	
	
	$pay_list = $pdo_conn->prepare("SELECT  `date`,`customer_name`,sum(`today_collection`) as todaycoll,`company_name` FROM paymentfollowups WHERE executive ='".$_POST['staffcreation_id']."' AND  delete_status != '1' and MONTH(date) = MONTH(CURRENT_DATE()) group by customer_name");
	$pay_list->execute();
	$paymentlist = $pay_list->fetchAll();	
	...
	<table width="100%" cellspacing="0" cellpadding="0" >
	<tr> 
	<td height="37" align="center" class="style10"><strong><b><h4>&nbsp;<?php echo ucfirst($mainlists['staff_name']); ?> - Collection Target <?php echo $cr_month ?> ' <?php echo $cr_year ?></h4></b></strong></td>
	</tr>
	</table>	               
	...
	{		
		...
		$executive = $_POST['staffcreation_id'];
		?>
		<tr>	
		<td align="left" height="25" class="style2">&nbsp;<?php echo $i = $i+1; ?>.</td>
		<td align="left" class="style2">&nbsp;<?php echo date("d-m-Y",strtotime($value['date'])); ?></td>	
		...
		?>	
		<td align="left" class="style2">&nbsp;<?php echo $customer_profile[0]['company_name'];?></td> 	
		<td align="center" class="style2">&nbsp;<?php echo payment_creation($Customer_name,$executive); ?></td> 
		<td align="center" class="style2">&nbsp;<?php echo number_format($value['todaycoll'],2);?></td> 		
		</tr>
	<?php	
	}
	?>
