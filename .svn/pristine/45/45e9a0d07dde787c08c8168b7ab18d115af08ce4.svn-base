   ...
   ?>   <div class="box-body">
    ...
    <small> </small>
		...
		<div class="col-md-2 col-lg-2">
			<h5><br></h5>
			<div class="form-group">
				<input type="button" name="" id="" onclick="paymentfollowup_list()" class="form-control btn-info" value="GO" >
		  ...
		  <div class="table-responsive">               
			<table id="example" class="table table-bordered table-hover table-striped display nowrap margin-top-10 w-p100">
			  ...
			  <thead>
				...
				$paymentcollection= $pdo_conn->prepare("SELECT * from paymentcollection where   $query order by paymentcreation_id desc");
				$paymentcollection->execute();
				$paymentcollection_list = $paymentcollection->fetchAll();

				  $roll_id=1;
				?>

			  <tbody>						
              <?php foreach($paymentcollection_list as $value){
				 
				 $staff_latitude=$value['latitude'];
			 	$staff_longitude=$value['longitude'];
				echo $voucher_image=$value['voucher_image'];
			 ?>
				...
				<td><a href="javascript:pdf_view('<?php echo $image_path."payment_images/".$voucher_image; ?>');" title="View Material" ><img <?php if($voucher_image!='') { ?> src="<?php echo $image_path."payment_images/".$voucher_image; ?>" <?php } else { ?>  src="images/images.jpg"  <?php }  ?> style="height:60px; width:60px;"  ></a></td>
				<td><?php if(($staff_longitude!='')&&($staff_latitude!='')) {?>
                  <div align="center">
                  <a onclick="print_google_map('paymentcollection/map.php?staff_latitude=<?php echo $staff_latitude; ?>&staff_longitude=<?php echo $staff_longitude; ?>');" target="new">
                  <img src="images/map.png" height="30" width="30">
                  </a></div><?php }?></td>
				<td><?php echo number_format($value['coll_amount'],2); ?>   </td>
...
function paymentfollowup_list(){
	...
	window.location.href = "index.php?file=paymentcollection/list&"+format;
