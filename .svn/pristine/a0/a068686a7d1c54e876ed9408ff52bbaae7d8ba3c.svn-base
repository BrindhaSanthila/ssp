...
<?php
	...
	$main_list = $pdo_conn->prepare("SELECT * FROM unit_creation WHERE unit_id ='".$_GET['unit_id']."'");
	$main_list->execute();
	$mainlist = $main_list->fetchAll();	
		$unit_name		    = $mainlist[0]['unit_name'];
...
?>
<script language="javascript" type="text/javascript" src="unit/unit.js"></script>
					...
					<form class="was-validated" name="staffentry" autocomplete="off">
						...
						<div class="col-md-12 col-lg-8 ">
								<div class="form-group">
									   <h5>Unit Name</h5>
									   <div class="controls">
											<input type="text" name="unit_name" id="unit_name" value="<?php echo $unit_name ?>" class="form-control" onchange="validation(this.id)" >
										</div>
								</div>
							</div>
							...
							<div class="col-md-12"><br><br>
									<a href="index.php?file=unit/list" class="float-left btn btn-primary">Cancel</a>
									<?php if(isset($_GET['unit_id'])){?>
									<button type="button" id="add" class="float-right btn btn-success" onclick="unitcreation_cu('<?php echo $_GET['unit_id']?>','Update')">Update</button>
									<?php }else{?>
									<button type="button" id="add" class="float-right btn btn-success" onclick="unitcreation_cu('','Add')"> Save</button>
									<?php }?>
							</div>
